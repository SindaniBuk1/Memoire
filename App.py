...
if dimension==1:
	...
	f_s=sympify(fct.lower())
	if st.checkbox("Voir la primitive"):
		txt="\iiint\int ("+latex(sympify(fct.lower()))+")dxdydzdt="+latex(integrate(f_s,x,y,z,t))
		st.latex(txt)
	iterations=st.sidebar.number_input("Nombre d'itérations",3,1000000,100)
	if st.button("Calculer","compute",help="Appuyez pour calculer l'intégrale"):
		I,df=mc_4d(f,iterations)
		txt="\int \iiint_{[0,1]^{ "+str(dimension)+"}}"+"("+latex(sympify(fct.lower()))+")dxdydzdt="+str(I)
		st.latex(txt)
elif dimension==5:
	...
	f_s=sympify(fct)
	if st.checkbox("Voir la primitive"):
		txt="\iint\iiint("+latex(sympify(fct.lower()))+")dxdydzdtdu="+latex(integrate(f_s,x,y,z,t,u))
		st.latex(txt)
	iterations=st.sidebar.number_input("Nombre d'itérations",3,1000000,100)
	if st.button("Calculer","compute",help="Appuyez pour calculer l'intégrale"):
